...
Visualization #5:
Interactive Geospatial map of Vegetarian & Vegan Restaurant locations across the United States or in a given state / city
Data: Vegetarian & Vegan Restaurant Across America | Data World
https://data.world/datafiniti/vegetarian-vegan-restaurants/workspace/file?filename=vegetarian_restaurants_US_datafiniti%2Fvegetarian_restaurants_US_datafiniti.csv

Healthiest States: Colorado, Massachusetts, Washington DC, Hawaii
...
# Read the CSV file
data <- read_csv("/Users/justin/Downloads/vegetarian_restaurants_US_datafiniti_vegetarian_restaurants_US_datafiniti.csv")

# Filter the data for restaurants in Colorado
CO_data <- data %>%
  filter(province == "CO")

# Define the Colorado bounding box
CO_bbox <- list(lat = c(37, 41), lng = c(-109, -102))

# Create the leaflet map
leaflet_map <- leaflet(data = CO_data) %>%
  setView(lng = -105.5, lat = 39, zoom = 7) %>% # Center the map on Colorado
  fitBounds(CO_bbox$lng[1], CO_bbox$lat[1], CO_bbox$lng[2], CO_bbox$lat[2]) %>% # Fit the map bounds to Colorado
...
# Filter the data for restaurants in Massachusetts
MA_data <- data %>%
  filter(province == "MA") 

# Define the Massachusetts bounding box
MA_bbox <- list(lat = c(41.2378, 42.8867), lng = c(-73.5081, -69.9286))

# Create the leaflet map
leaflet_map <- leaflet(data = MA_data) %>%
  setView(lng = -71.5, lat = 42, zoom = 8) %>% # Center the map on Massachusetts
  fitBounds(MA_bbox$lng[1], MA_bbox$lat[1], MA_bbox$lng[2], MA_bbox$lat[2]) %>% # Fit the map bounds to Massachusetts
  addProviderTiles("CartoDB.Positron") %>%
  addCircleMarkers(
    color = "darkgreen",
    lng = ~long,
...
# Filter the data for restaurants in Washington D.C.
DC_data <- data %>%
  filter(city == "Washington Dc") 

# Define the Washington D.C. bounding box
DC_bbox <- list(lat = c(38, 39), lng = c(-77, -76))

# Create the leaflet map
leaflet_map <- leaflet(data = DC_data) %>%
  setView(lng = -77.0369, lat = 38.9072, zoom = 12) %>% # Center the map on Washington D.C.
  fitBounds(DC_bbox$lng[1], DC_bbox$lat[1], DC_bbox$lng[2], DC_bbox$lat[2]) %>% # Fit the map bounds to Washington D.C.
  addProviderTiles("CartoDB.Positron") %>%
  addCircleMarkers(
    color = "darkgreen",
    lng = ~long, 
...
# Filter the data for restaurants in Hawaii
HI_data <- data %>%
  filter(province == "HI") 

# Define the Hawaii bounding box
HI_bbox <- list(lat = c(18.8655, 22.2356), lng = c(-160.2441, -154.8066))

# Create the leaflet map
leaflet_map <- leaflet(data = HI_data) %>%
  setView(lng = -157.8583, lat = 21.3069, zoom = 7) %>% # Center the map on Hawaii
  fitBounds(HI_bbox$lng[1], HI_bbox$lat[1], HI_bbox$lng[2], HI_bbox$lat[2]) %>% # Fit the map bounds to Hawaii
  addProviderTiles("CartoDB.Positron") %>%
  addCircleMarkers(
    color = "darkgreen",
    lng = ~long, 
...
Unhealthiest States: Mississippi, West Virginia, Arkansas
...
# Filter the data for restaurants in Mississippi
MS_data <- data %>%
  filter(province == "MS")

# Define the Mississippi bounding box
MS_bbox <- list(lat = c(30.1739, 34.9961), lng = c(-91.6550, -88.0979))

# Create the leaflet map
leaflet_map <- leaflet(data = MS_data) %>%
  setView(lng = -89.3985, lat = 32.3547, zoom = 7) %>% # Center the map on Mississippi
  fitBounds(MS_bbox$lng[1], MS_bbox$lat[1], MS_bbox$lng[2], MS_bbox$lat[2]) %>% # Fit the map bounds to Mississippi
  addProviderTiles("CartoDB.Positron") %>%
  addCircleMarkers(
    color = "darkgreen",
    lng = ~long,
...
# Filter the data for restaurants in West Virginia
WV_data <- data %>%
  filter(province == "WV")

# Define the West Virginia bounding box
WV_bbox <- list(lat = c(37.2015, 40.6388), lng = c(-82.6447, -77.7190))

# Create the leaflet map
leaflet_map <- leaflet(data = WV_data) %>%
  setView(lng = -80.4549, lat = 38.5976, zoom = 7) %>% # Center the map on West Virginia
  fitBounds(WV_bbox$lng[1], WV_bbox$lat[1], WV_bbox$lng[2], WV_bbox$lat[2]) %>% # Fit the map bounds to West Virginia
  addProviderTiles("CartoDB.Positron") %>%
  addCircleMarkers(
    color = "darkgreen",
    lng = ~long,
...
# Filter the data for restaurants in Arkansas
AR_data <- data %>%
  filter(province == "AR")

# Define the Arkansas bounding box
AR_bbox <- list(lat = c(33.0041, 36.4996), lng = c(-94.6179, -89.6448))

# Create the leaflet map
leaflet_map <- leaflet(data = AR_data) %>%
  setView(lng = -92.1998, lat = 34.9697, zoom = 7) %>% # Center the map on Arkansas
  fitBounds(AR_bbox$lng[1], AR_bbox$lat[1], AR_bbox$lng[2], AR_bbox$lat[2]) %>% # Fit the map bounds to Arkansas
  addProviderTiles("CartoDB.Positron") %>%
  addCircleMarkers(
    color = "darkgreen",
    lng = ~long,
