...
states_11@data <- states_11@data %>%
  select(State, obesity_11)

labels_11 <- sprintf(
  "<strong>%s</strong><br/>%g percent",
  states_11@data$State, states_11@data$obesity_11
) %>% lapply(htmltools::HTML)

o_11 <- leaflet(states_11) %>%
  setView(-96, 37.8, 2.25) %>%
...
# Add legend
o_11 <- o_11 %>%
  addLegend(pal = pal, values = ~obesity_11, opacity = 0.7, 
            title = "Percentage of Obesity (%)", 
            position = "bottomright")

# Add state abbreviations and centers to states_11@data
...
states_11@data <- left_join(states_11@data, statenames, by = "State")

states_11@data$state.center.x <- ifelse(states_11@data$State == "Alaska", custom_coordinates$custom_lng[1], states_11@data$state.center.x)
states_11@data$state.center.y <- ifelse(states_11@data$State == "Alaska", custom_coordinates$custom_lat[1], states_11@data$state.center.y)

states_11@data$state.center.x <- ifelse(states_11@data$State == "Hawaii", custom_coordinates$custom_lng[2], states_11@data$state.center.x)
states_11@data$state.center.y <- ifelse(states_11@data$State == "Hawaii", custom_coordinates$custom_lat[2], states_11@data$state.center.y)
...
states_16@data <- states_16@data %>%
  select(State, obesity_16)

labels_16 <- sprintf(
  "<strong>%s</strong><br/>%g percent",
  states_16@data$State, states_16@data$obesity_16
) %>% lapply(htmltools::HTML)
...
# Add legend
o_16 <- o_16 %>%
  addLegend(pal = pal, values = ~obesity_16, opacity = 0.7, 
            title = "Percentage of Obesity (%)", 
            position = "bottomright")

# Add state abbreviations and centers to states_16@data
statenames <- data.frame(
  State = state.name,
  state.abb = state.abb,
  state.center.x = state.center$x,
...
states_16@data <- left_join(states_16@data, statenames, by = "State")

states_16@data$state.center.x <- ifelse(states_16@data$State == "Alaska", custom_coordinates$custom_lng[1], states_16@data$state.center.x)
states_16@data$state.center.y <- ifelse(states_16@data$State == "Alaska", custom_coordinates$custom_lat[1], states_16@data$state.center.y)

states_16@data$state.center.x <- ifelse(states_16@data$State == "Hawaii", custom_coordinates$custom_lng[2], states_16@data$state.center.x)
states_16@data$state.center.y <- ifelse(states_16@data$State == "Hawaii", custom_coordinates$custom_lat[2], states_16@data$state.center.y)
...
states_21@data <- states_21@data %>%
  select(State, obesity_21)

labels_21 <- sprintf(
  "<strong>%s</strong><br/>%g percent",
  states_21@data$State, states_21@data$obesity_21
) %>% lapply(htmltools::HTML)
...
# Add legend
o_21 <- o_21 %>%
  addLegend(pal = pal, values = ~obesity_21, opacity = 0.7, 
            title = "Percentage of Obesity (%)", 
            position = "bottomright")

# Add state abbreviations and centers to states_21@data
statenames <- data.frame(
  State = state.name,
  state.abb = state.abb,
  state.center.x = state.center$x,
...
states_21@data <- left_join(states_21@data, statenames, by = "State")

states_21@data$state.center.x <- ifelse(states_21@data$State == "Alaska", custom_coordinates$custom_lng[1], states_21@data$state.center.x)
states_21@data$state.center.y <- ifelse(states_21@data$State == "Alaska", custom_coordinates$custom_lat[1], states_21@data$state.center.y)

states_21@data$state.center.x <- ifelse(states_21@data$State == "Hawaii", custom_coordinates$custom_lng[2], states_21@data$state.center.x)
states_21@data$state.center.y <- ifelse(states_21@data$State == "Hawaii", custom_coordinates$custom_lat[2], states_21@data$state.center.y)
...
# Wrap each leaflet output in a div container
o_11_div <- tags$div(style = "display:inline-block; width: 32%; height: 300px;", o_11)
o_16_div <- tags$div(style = "display:inline-block; width: 32%; height: 300px;", o_16)
o_21_div <- tags$div(style = "display:inline-block; width: 32%; height: 300px;", o_21)

# Display the containers side by side
tagList(o_11_div, o_16_div, o_21_div)
...
data.frame(height = rep(height, each= length(weight)), 
  ...
  mutate(bmi= weight*703/(height^2)) -> df
...
df |>
  ...
  mutate(category = factor(category, levels = c(
    'Underweight', 'Healthy',
    'Overweight', 'Obese', 'Severely Obese')))-> df

df |>
  mutate(feet = floor(height / 12),
         inches = height - (12 * feet), 
         height_label = paste0(feet, "'", inches, '"')) -> df


df |> 
  mutate(height_label = factor(height_label, levels=unique(df$height_label))) -> df

p <- ggplot(df, aes(x = factor(weight), y = height_label, fill = category,
                    text = paste('Weight:', factor(weight), 'lbs',
                                 '<br>Height: ', height_label,
                                 '<br>BMI: ', round(bmi),
                                 '<br>Category:', category)), alpha=0.8) + theme_bw() + 
  geom_tile(color='black', linewidth=0.5) + 
  geom_text(aes(label = round(bmi)), size=3) + 
  scale_y_discrete(limits=rev) +
  labs(x = 'Weight (lbs)', y = 'Height', fill = '') +
  scale_fill_manual(breaks = levels(df$category),
...
plot <- plot_ly(data = diet_data, x = ~SurveyPeriod, y = ~WeightLossDiet, type = "scatter", mode = "lines+markers", name = "Weight Loss or Low Calorie Diet", line = list(color = "#184E77", width = 4))
plot <- plot %>% add_trace(y = ~DiabeticDiet, name = "Diabetic Diet", line = list(color = "#168AAD", width = 4))
plot <- plot %>% add_trace(y = ~LowCarbDiet, name = "Low Carbohydrate Diet", line = list(color = "#B5E48C", width = 4))
plot <- plot %>% add_trace(y = ~LowFatDiet, name = "Low Fat or Low Cholesterol Diet", line = list(color = "#52B69A", width = 4))

plot <- plot %>% layout(title = "Special Diets Among Adults in the US from 2007 to 2018",
                         xaxis = list(title = "Survey Period", automargin = TRUE, title_standoff = 25),
                         yaxis = list(title = "Percentage (%)"),
                         hovermode = 'closest',
                         hoverlabel = list(bgcolor = "white"),
                         legend = list(orientation = "v", xanchor = "left", yanchor = "top", x = 1, y = 1)) # Update the legend parameters here
...
p <- df |>
  ...
  theme(axis.title.x = element_text(margin = margin(t = 16, r = 0, b = 0, l = 0))) # Adjust the margin here

interactive_plot <- ggplotly(p, tooltip = c("x", "fill", "count"))
...
![](/Users/justin/Downloads/ProcessCodebook.JPG){#id .class width=200}
